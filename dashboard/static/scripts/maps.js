...
var plotCircleAnimation = function(item, map, list, color, value){
    var itemCircle = new google.maps.Circle({
        strokeColor: color,
        strokeOpacity: 0.8,
        strokeWeight: 2,
        ...
        map: map,
        center: {lat: latitudes[item], lng: longitudes[item]},
        radius: Math.sqrt(value) * 100,
    });
    circlesAnimation.push(itemCircle);
}
...
function myMap() {
    ...
    /**var intervalID = setInterval(function(){
        week += 1;
        for (var i = 0; i < zikaPerTime.length; i++){
            for (var local in zikaPerTime[i]){
                plotCircleAnimation(local, map, list, color, ZikaPerTime[i][local])
            }
            week += 1
        }
    },800);**/
